...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
  ...
  override func viewDidLoad() {
    ...
    do {
      ...
      guard let resNet50Model = try? VNCoreMLModel(for: SqueezeNet().model) else {
    ...
    func updateThreshholdLabel () {
        self.threshholdLabel.text = "Sensitivity: " + String(format: "%.2f", recognitionThreshold)
    }
  ...
  func handleClassifications(request: VNRequest, error: Error?) {
    ...
    if  classifications.range(of: "toilet") != nil ||
        ...
        DispatchQueue.main.async {
            self.resultView.text = "Good Morning!!!"
        }
    }
  }
}
